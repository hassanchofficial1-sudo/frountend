...
export default function NewElectricityPage() {
  ...
  const onSubmit = async (data: ElectricityForm) => {
    ...
    try {
      ...
      if (result.success) {
        addToast({ type: 'success', title: 'Bill recorded', message: result.message })
        router.push('/electricity')
      } else {
        addToast({ type: 'error', title: 'Failed to record bill', message: result.error })
      }
    } catch (error) {
      addToast({ type: 'error', title: 'Error', message: 'Failed to record bill' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <BackButton />
      <div>
        <h1 className="text-2xl font-bold text-secondary-900 dark:text-secondary-100">New Electricity Bill</h1>
        <p className="text-secondary-500 dark:text-secondary-400 mt-1">Record electricity bill for your shop</p>
      </div>
