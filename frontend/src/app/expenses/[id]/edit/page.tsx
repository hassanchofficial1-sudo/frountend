...
export default function EditExpensePage() {
  ...
  const onSubmit = async (data: ExpenseForm) => {
    ...
    try {
      ...
      if (result.success) {
        addToast({ type: 'success', title: 'Expense updated', message: result.message })
        router.push('/expenses')
      } else {
        addToast({ type: 'error', title: 'Failed to update expense', message: result.error })
      }
    } catch (error) {
      addToast({ type: 'error', title: 'Error', message: 'Failed to update expense' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <BackButton />
      <div>
        <h1 className="text-2xl font-bold text-secondary-900">Edit Expense</h1>
        <p className="text-secondary-500 dark:text-secondary-400 mt-1">Update expense details</p>
      </div>
