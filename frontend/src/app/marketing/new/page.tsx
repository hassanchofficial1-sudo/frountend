...
export default function NewMarketingPage() {
  ...
  const onSubmit = async (data: MarketingForm) => {
    ...
    try {
      ...
      if (result.success) {
        addToast({ type: 'success', title: 'Campaign recorded', message: result.message })
        router.push('/marketing')
      } else {
        addToast({ type: 'error', title: 'Failed to record campaign', message: result.error })
      }
    } catch (error) {
      addToast({ type: 'error', title: 'Error', message: 'Failed to record campaign' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <BackButton />
      <div>
        <h1 className="text-2xl font-bold text-secondary-900 dark:text-secondary-100">New Marketing Campaign</h1>
        <p className="text-secondary-500 dark:text-secondary-400 mt-1">Record advertising spend and performance</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" isLoading={loading}>
            <Save className="w-4 h-4 mr-2" />Record Campaign
          </Button>
        </div>
      </form>
    </div>
