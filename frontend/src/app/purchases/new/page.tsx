...
export default function NewPurchasePage() {
  ...
  const onSubmit = async (data: PurchaseForm) => {
    ...
    try {
      ...
      if (result.success) {
        addToast({ type: 'success', title: 'Purchase created', message: result.message })
        router.push(`/purchases`)
      } else {
        addToast({ type: 'error', title: 'Failed to create purchase', message: result.error })
      }
    } catch (error) {
      addToast({ type: 'error', title: 'Error', message: 'Failed to create purchase' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <BackButton />
      <div>
        <h1 className="text-2xl font-bold text-secondary-900">New Purchase</h1>
        <p className="text-secondary-500 dark:text-secondary-400 mt-1">Create a new purchase order</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" isLoading={loading}>
            <Save className="w-4 h-4 mr-2" />Create Purchase
          </Button>
        </div>
      </form>
    </div>
